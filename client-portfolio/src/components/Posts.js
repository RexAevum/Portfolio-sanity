                        ...
                        <article>
                            <Link to={`/post/${post.slug.current}`} key={post.slug.current}>
                                <span className="block h-64 relative rounded shadow leading-snug bg-blue-50 border-l-8 border-green-400" key={index}>
                                    <img src={post.mainImage && post.mainImage.asset.url || stock} alt={post.mainImage && post.mainImage.alt} className="w-full h-full rounded-r object-cover absolute inline"  />
